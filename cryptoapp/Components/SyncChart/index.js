...
export function SyncChart() {
  ...
  const chartData = useMemo(() => {
    ...
    return coinHistory.prices.map((priceData, index) => ({
      date: formatDate(priceData[0]),
    ...
    return (
      <div className="mb-8">
        <div className="flex justify-between items-center mb-2">
          ...
          <Link
            href={{
              pathname: `/forecast/${CoinID}/${forecastType}`,
              query: { timeframe: days },
            }}
          >
            <Button
        ...
        <div className="h-[200px] w-full  bg-slate-200  dark:bg-white/5 backdrop-blur-sm rounded-lg p-2">
          <ResponsiveContainer>
            <AreaChart
              data={chartData}
              // syncId="anyId"
            ...
            >
              ...
              <Brush
                ...
                startIndex={Math.max(
                  0,
                  chartData.length - Math.floor(chartData.length * 0.33)
                )}
                endIndex={chartData.length - 1}
                className={`custom-brush brush-${type}`}
              >
                <AreaChart>
  ...
  return (
    <div className="border bg-slate-100 dark:border dark:border-stone-700 dark:bg-slate-950 rounded-md p-4">
      <div className="flex justify-between w-full mb-6 gap-4">
        {timeButtons.map(({ value, label }) => (
          <button
            ...
            onClick={() => {
              setDays(value);
              setLastClickedButton(value);
            }}
            className={`
              flex-1 h-10 px-4 rounded-lg font-medium
              transition-all duration-200 ease-in-out
              ${
                lastClickedButton === value
                  ? "bg-amber-500 text-white dark:bg-amber-600 shadow-lg transform scale-105"
                  : "bg-slate-200 text-slate-700 dark:bg-slate-900 dark:text-slate-300 hover:bg-slate-300 dark:hover:bg-slate-700"
              }
      ...
      {renderChart("price")}
      {renderChart("marketCaps")}
      {renderChart("totalVolumes")}
    </div>
  );
}
